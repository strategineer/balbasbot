...
const TEAMS_SELECT_SQL =
  "SELECT name, wins, losses, draws, created_on, retired_on FROM BLOOD_BOWL_TEAM ORDER BY created_on DESC";
const TEAM_SELECT_SQL =
  "SELECT name, wins, losses, draws, created_on, retired_on FROM BLOOD_BOWL_TEAM WHERE retired_on is null ORDER BY created_on DESC LIMIT 1";
const TEAM_INSERT_SQL =
  "INSERT INTO BLOOD_BOWL_TEAM (name, wins, losses, draws, created_on, retired_on) VALUES (?, 0, 0, 0, datetime('now'), null)";
const TEAM_UPDATE_STATS_SQL =
  "UPDATE BLOOD_BOWL_TEAM SET wins=?, draws=?, losses=? WHERE name=?";
const TEAM_RETIRE_SQL =
  "UPDATE BLOOD_BOWL_TEAM SET retired_on=datetime('now') WHERE name=?";
...
function updateStats(client, target, delta_wins, delta_draws, delta_losses) {
  database.db.serialize(() => {
    database.db.get(TEAM_SELECT_SQL, [], (err, row) => {
      ...
      if (row) {
        database.db.run(
          TEAM_UPDATE_STATS_SQL,
          [
            row.wins + delta_wins,
            row.draws + delta_draws,
            row.losses + delta_losses,
            row.name,
          ],
          (err) => {}
        );
